...
def segmentStableNotesRegions(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    for i in range(offset, f0cent.size):
        stds = np.append(stds, np.std(f0cent[i-offset:i+1]))
    ...
    if len(stables) > 0:
        ...
        for s in stables:
            if s != (sequence[0] + index):
                all_segments.append([sequence[0], sequence[len(sequence) - 1]])
                sequence = []
            sequence.append(s)
            index = len(sequence)
