...
def computeODF(inputFile, window, M, N, H):
    ...
    odf_low = odf(energy(low_band))
    odf_high = odf(energy(high_band))  
    
    ODF = np.append([odf_low], [odf_high], axis=0)
    ODF = np.transpose(ODF)
    ...
    plt.subplot(212)
    plt.plot(frmTime, odf_low, color='blue', label='ODF Low')
    plt.plot(frmTime, odf_high, color='green', label='ODF High')                                             
